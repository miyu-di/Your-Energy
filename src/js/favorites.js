...
const emptyMessage = document.querySelector('.favorites-empty-message');
...
if (favoritesList) {
  favoritesList.addEventListener('click', e => {
    ...
    if (btn.classList.contains('fav-trash-btn')) {
      deleteFavorite(id);
    }

    if (btn.classList.contains('fav-start-btn')) {
      const favorites = JSON.parse(localStorage.getItem(LS_FAV_KEY)) || [];
  ...
  return data
    ...
    <li class="fav-card">
      ...
      <div class="fav-title-row">
         ...
         <h3 class="fav-name">${capitalize(item.name)}</h3>
      </div>
      <ul class="fav-info">
         <li class="fav-text">
            <span class="info-label">Burned calories:</span> 
            <span class="info-value">${item.burnedCalories} / ${item.time} min </span> 
         </li>
         <li class="fav-text">
            <span class="info-label">Body part:</span> 
            <span class="info-value">${capitalize(item.bodyPart)}</span> 
         </li>
         <li class="fav-text">
            <span class="info-label">Target:</span> 
            <span class="info-value">${capitalize(item.target)}</span> 
         </li>
      </ul>
    </li>
  `
