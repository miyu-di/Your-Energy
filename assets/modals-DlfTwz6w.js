import{getExerciseById as h,patchRating as y}from"./api-CA10J7K2.js";import"./vendor-D1yz-ZLW.js";const o=document.querySelector("#exercise-modal"),g=document.querySelector(".exercises-list");let d=null;g&&g.addEventListener("click",async t=>{const e=t.target.closest(".exercise-start-btn");if(e)try{const a=await h(e.dataset.id);a&&v(a)}catch(a){console.error(a)}});function v(t){d=t,o.innerHTML=w(t),o.classList.remove("is-hidden"),document.body.classList.add("no-scroll"),window.addEventListener("keydown",f),E(t)}function E(t){const e=document.querySelector(".btn-favorite");if(!e)return;const r=(JSON.parse(localStorage.getItem("favorites"))||[]).some(s=>s._id===t._id);u(e,r),e.addEventListener("click",()=>{const s=JSON.parse(localStorage.getItem("favorites"))||[],n=s.findIndex(i=>i._id===t._id);n!==-1?(s.splice(n,1),u(e,!1)):(s.push(t),u(e,!0)),localStorage.setItem("favorites",JSON.stringify(s))})}function u(t,e){e?t.innerHTML='Remove from favorites <span class="heart-active">❤</span>':t.innerHTML='Add to favorites <span class="heart-icon">❤</span>'}function p(){if(o.classList.add("is-hidden"),o.innerHTML="",document.body.classList.remove("no-scroll"),window.removeEventListener("keydown",f),window.location.pathname.includes("favorite")){const t=new Event("favorites-updated");window.dispatchEvent(t)}}function f(t){t.code==="Escape"&&p()}o&&o.addEventListener("click",t=>{(t.target===o||t.target.closest("#modal-close"))&&p()});document.addEventListener("click",t=>{t.target.classList.contains("btn-rating")&&(o.innerHTML=$(d._id),L())});function L(){let t=0;const e=document.querySelectorAll(".star-input"),a=document.querySelector("#rating-value"),r=document.getElementById("rating-form");e.forEach(s=>{s.addEventListener("click",n=>{t=Number(n.target.dataset.value),a.textContent=`${t}.0`,e.forEach(i=>i.classList.toggle("active",i.dataset.value<=t))})}),r&&(r.onsubmit=async s=>{var l,c;s.preventDefault();const{email:n,comment:i}=s.target.elements;if(t===0)return alert("Please select a rating!");try{await y(d._id,{rate:t,email:n.value.trim(),comment:i.value.trim()}),alert("Thank you for your feedback!"),v(d)}catch(m){alert(((c=(l=m.response)==null?void 0:l.data)==null?void 0:c.message)||"Error submitting rating")}})}function w(t){const{gifUrl:e,name:a,rating:r,target:s,bodyPart:n,equipment:i,popularity:l,burnedCalories:c,description:m,_id:b}=t;return`
...
//# sourceMappingURL=modals-DlfTwz6w.js.map
